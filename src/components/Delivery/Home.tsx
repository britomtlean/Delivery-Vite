                ...
                <>
                    <header className="w-full min-h-[10vh] py-4 px-[20%] flex justify-between items-center mb-4 bg-[rgb(48,62,83)]">
                        ...
                        <div className='flex flex-col gap-1 p-2 rounded-2xl'>
                            <h3 className="text-white font-bold text-[0.8rem]">{user?.nome}</h3>
                            <button onClick={() => {setUser((prev) => {deleteToken(); return null})}}>Logout</button>
                        </div>
                    </header>

                    <div className="h-full w-full max-w-[1280px] flex justify-center items-start pt-4">
                        {renderComponente()}
                    </div>
                </>
            ) : (
